...
class ServerlessTemplateContext {
	...
	constructor(template, position){
		this.__template = deepCopy(template);
		this.__position = position;
		this.__ancestry = this.__createAncestry();
	}
	...
	__createAncestry(){
		...
		this.__position.forEach((childKey) =>{
			const element = {
				key: childKey,
	...
	/**
	 * @returns {Object} - the object containing this function call as a child
	 */
	get templateFunctionParent(){
		return this.ancestryFunction.parent.value;
	}

	/**
	 * @returns {Object} - same as templateFunctionParent but without the actual function call prop (useful for including the siblings in the return statement)
	 */
	get templateFunctionSiblings(){
		const parent = { ...this.templateFunctionParent };
		delete parent[this.ancestryFunction.key];
		return parent;
	}
	 ...
	 */
	get root(){
		return this.template[this.__position[0]];
	}
}
...
class ServerlessPluginTemplateFunctionCalls {
	constructor(serverless, options){
		...
		this.hooks = {
			"before:deploy:function:initialize": () => this.processTemplate(),
	...
	loadModules(modulePaths, config){
		...
		modulePaths.forEach((modulePath) => {
			...
			try {
				__dirname = this.resolvePath("./");
				process.chdir(this.resolvePath("./"));

				const absoluteModulePath = this.resolvePath(modulePath);

				// require module file either as an absolute or relative path
				if(fs.existsSync(absoluteModulePath)){
					modules[moduleFilename] = require(absoluteModulePath);
				} else {
	...
	executeFunction(functionName, functionArguments, template, position, functions, config){
		...
		try {
			...
			})(...(functionArguments.map(deepCopy)));
		} catch(err) {
	...
	processTemplate(){
		...
		// create proxy object to control traversable properties of the template
		let proxyTemplate = {};
		config.roots.forEach((root) => {
			Object.defineProperty(proxyTemplate, root, {
				enumerable: true,
				set: (val) => { this.template[root] = val; },
				get: () => this.template[root]
			})
		});

		// recursively execute functions for each template root specified in the config
		this.inplaceFormatField(proxyTemplate, [], functions, config);
	}
}
